...
const userUrl = 'http://ec2-100-26-220-146.compute-1.amazonaws.com/o-calm/wp-json';

const usersMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case FETCH_USERS: {
      axios.get(`${apiUrl}/users?per_page=100`)
        .then((response) => {
    ...
    case VERIF_LOGIN: {
      ...
      axios.post(`${apiUrl}/token?username=${username}&password=${password}`)
        .then((response) => {
    ...
    case VERIF_SESSION: {
      ...
      const verifySession = new Promise((resolve, reject) => {
        if (localStorage.token) {
          axios({
            method: 'post',
            url: `${userUrl}/jwt-auth/v1/token/validate`,
    ...
    case SEND_SUBSCRIBE: {
      ...
      axios.post(`${apiUrl}/users/register`, subArray)
        .then((response) => {
    ...
    case SEND_FAVORITES: {
      ...
      const sendFav = new Promise((resolve, reject) => {
        axios({
          method: 'post',
          url: `${userUrl}/ocalm-settings/v1/video/favorite`,
          headers: { Authorization: `Bearer ${token}` },
          data: { post_id },
        })
          .then(resolve)
    ...
    case IMPORT_FAVORITES: {
      ...
      const saveFav = new Promise((resolve, reject) => {
        axios({
          method: 'get',
          url: `${userUrl}/ocalm-settings/v1/video/favorite?per_page=100`,
          headers: { Authorization: `Bearer ${token}` },
        })
          .then(resolve)
          .catch(reject);
